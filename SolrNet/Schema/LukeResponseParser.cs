...
namespace SolrNet.Schema {
    public class LukeResponseParser {
        ...
        /// <summary>
        /// Creates for each element in the LukeResponse based on the /admin/luke response
        /// a new <see cref="SolrField"/>.
        /// </summary>
        /// <param name="lukeResponse"></param>
        /// <returns></returns>
        public LukeResponse Parse(XDocument lukeResponse)
        {
            ...
            foreach (var fieldNode in lukeResponse.XPathSelectElements("/response/lst[@name='fields']/lst"))
            {
                ...
                if (attributeFieldName != null)
                {
                    ...
                    var solrFieldType = new SolrFieldType(typeName, null);
