...
namespace SolrNet.Schema
{
    ...
    /// </summary>
    public class SolrSchemaMappingValidationManager : ISolrSchemaMappingValidationManager
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="SolrSchemaMappingValidationManager"/> class.
        /// </summary>
        ...
        public SolrSchemaMappingValidationManager(IReadOnlyMappingManager mappingManager, ISolrSchemaParser schemaParser)
        {
        ...
        public SolrSchemaMappingValidationResultSet Validate(Type type, XmlDocument solrSchemaXml)
        {
            var result = new SolrSchemaMappingValidationResultSet();
        ...
        private void ValidateUniqueKey(Type type, SolrSchemaMappingValidationResultSet resultSet)
        {
            if (solrSchema.UniqueKey != null) // Validate that the unique key in the schema is present in the type.
            {
                if (!mappingManager.GetUniqueKey(type).Value.Equals(solrSchema.UniqueKey))
                    resultSet.Errors.Add(new SolrSchemaMappingValidationError(
        ...
        /// <param name="resultSet">The validation result set to add errors and warnings to</param>
        private void ValidateMappedPropertiesAreInSolrSchema(Type type, SolrSchemaMappingValidationResultSet resultSet)
        {
        ...
        /// <param name="resultSet">The validation result set to add errors and warnings to</param>
        private void ValidateRequiredFields(Type type, SolrSchemaMappingValidationResultSet resultSet)
        {
            foreach (SolrField solrField in solrSchema.SolrFields)
            {
