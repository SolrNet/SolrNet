...
{
    public class AtomicUpdatesCommandTests {
        private readonly ITestOutputHelper testOutputHelper;

        public AtomicUpdatesCommandTests(ITestOutputHelper testOutputHelper)
        {
		...
		public void AtomicUpdateSet() {
            ...
            var cmd = new AtomicUpdatesCommand("id", "0", updateSpecs, null);
            cmd.Execute(conn);
        ...
        public void AtomicUpdateWithParameter()
        {
            ...
            var updateSpecs = new AtomicUpdateSpec[] { new AtomicUpdateSpec("count", AtomicUpdateType.Inc, 3) };
            var cmd = new AtomicUpdatesCommand("id", "0", updateSpecs, parameters);
            cmd.Execute(conn);
            Assert.Equal(1, conn.postStream.Calls);
        }
        ...
        {
            var updateSpecs = new AtomicUpdateSpec[] { new AtomicUpdateSpec("count", AtomicUpdateType.Inc, "3") };
            Assert.Throws<ArgumentNullException>(() => new AtomicUpdatesCommand(null, "0", updateSpecs, null));
        }
        ...
        {
            var updateSpecs = new AtomicUpdateSpec[] { new AtomicUpdateSpec("count", AtomicUpdateType.Inc, "3") };
            Assert.Throws<ArgumentNullException>(() => new AtomicUpdatesCommand("id", null, updateSpecs, null));
        }

        [Fact]
        public void AtomicUpdateNullSpecHandledGracefully()
        {
            Assert.Throws<ArgumentNullException>(() => new AtomicUpdatesCommand("id", "0", null, null));
        }
        ...
        [Fact]
        public void AtomicUpdatesCommandNameInvariantCulture()
        {
            var conn = new Mocks.MSolrConnection();
            conn.postStream += new MFunc<string, string, Stream, IEnumerable<KeyValuePair<string, string>>, string>((url, contentType, content, param) => {
                string text = new StreamReader(content, Encoding.UTF8).ReadToEnd();
