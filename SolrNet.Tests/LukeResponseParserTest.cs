...
namespace SolrNet.Tests 
{
    ...
    public class LukeResponseParserTest 
    {
        ...
        public void Test_Parser_2()
        {
            BuildFakes();
            var parser = new LukeResponseParser();
            var response = parser.Parse(lukeResponse);

            Assert.AreEqual(4, response.SolrFields.Count);
            Assert.IsNotNull(response.SolrFields.Find(x => x.Name.Equals("foo_result")));
            Assert.IsNotNull(response.SolrFields.Find(x => x.Name.Equals("bar_result")));
            Assert.IsNotNull(response.SolrFields.Find(x => x.Name.Equals("id")));
            Assert.IsNotNull(response.SolrFields.Find(x => x.Name.Equals("baz")));
