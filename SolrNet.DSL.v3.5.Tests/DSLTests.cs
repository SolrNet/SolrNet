...
namespace SolrNet.DSL.v3._5.Tests {
	/// <summary>
...
");
		...
		public void QueryByRange_35() {
			...
			With.Mocks(mocks).Expecting(delegate {
				...
				Solr.Connection = conn;
				Solr.Query<TestDocumentWithId>().ByRange(x => x.Id, 123, 456).Run();
			});
		}
		...
		[Test]
		public void QueryByRange_AnotherSyntax_35() {
			var mocks = new MockRepository();
			var conn = mocks.CreateMock<ISolrConnection>();
			With.Mocks(mocks).Expecting(delegate {
				var query = new Dictionary<string, string> { { "q", "Id:[123 TO 456]" } };
				...
				Solr.Connection = conn;
				Solr.Query<TestDocumentWithId>().By(x => x.Id).Between(123).And(456).Run();
			});
		}
