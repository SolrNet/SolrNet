            ...
            foreach (XElement dateFacetingNode in node.Elements())
            {
                ...
                switch (name)
                {
                    ...
                    case "end":
                        ...
                        break;
                    case "counts":
                        foreach (var element in dateFacetingNode.Elements().Where(x => x.Name == "int").ToList())
                        {
                            var d = DateTimeFieldParser.ParseDate(element.Attribute("name").Value);
                            r.DateResults.Add(KV.Create(d, Int32.Parse(element.Value)));
                        }
                        break;
                    default:
